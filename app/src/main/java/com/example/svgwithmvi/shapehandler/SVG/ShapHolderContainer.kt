...
data class ShapeHolderContainer(val shapeHolder: ShapeHolder?) {
    ...
    fun unPackForMaker(list: MutableList<ShapeHolderContainer>) {

        if (this.actionType == ActionType.GROUP) {
            for (shapeHolder in pairedShapeHolders) {
                shapeHolder.unPackForMaker(list)
            }
        } else {
