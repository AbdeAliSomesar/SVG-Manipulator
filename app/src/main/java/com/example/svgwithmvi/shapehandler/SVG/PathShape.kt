...
class PathShape @Inject constructor(
    ...
    private fun addConstantX(width:Float) {
        for ((index, value) in moves.withIndex()) {
            if (index % 2 == 0)
                moves[index] += width

        }
        for ((index, value) in xCoordinates.withIndex()) {
            xCoordinates[index] += width
        }

    }
    private fun addConstantY(height:Float) {
        for ((index, value) in moves.withIndex()) {
            if (index % 2 != 0)
                moves[index] += height
        }

        for ((index, value) in yCoordinates.withIndex()) {
            yCoordinates[index] += height
        }
    }
