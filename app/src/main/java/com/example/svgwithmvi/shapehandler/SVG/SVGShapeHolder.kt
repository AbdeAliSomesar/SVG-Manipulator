...
class SVGShapeHolder @Inject constructor(var converter: Converter):ShapeHolder {
    ...
    private var bitmap:Bitmap = Bitmap.createBitmap(converter.getBitmapWidth(),converter.getBitmapHeight(),Bitmap.Config.ARGB_8888)
    private var minMax:Array<Float> = arrayOf(Float.MAX_VALUE, Float.MAX_VALUE, Float.MIN_VALUE,
        Float.MIN_VALUE)
    ...
    override fun updateHeightBy(percent: Float) {
      //  if(minMax[1] - percent >0)
       // {
        ...
        //update bitmap after removeConstantFromHeight()
    }
    ...
    override fun updateWidthBy(percent:Float) {
        ...
        //update bitmap after removeConstantFromWidth()
    ...
    override fun getSvgBody(): String {
        ...
        var finalTag = mainTag.replaceFirst("height?","" +( minMax[3]+100)).replaceFirst("width?","" + (minMax[2]+100))
